...
class FDAttention(InteractionLayer):
    def __init__(self, input_dim=128, emb_size=128, dropout_rate=0.1,
        ...
        super(FDAttention, self).__init__()
        # 记录初始参数
        logger.info(f"Initializing FDAttention with emb_max_pos_size={emb_max_pos_size}")

        self.device = device
        self.decoder = nn.Sequential(
            # nn.Linear(512, 256),  # 输入维度调整为512
            # nn.ReLU(True),
            #
            # nn.BatchNorm1d(256),
            nn.Linear(257, 128),
    ...
    def forward(self, *inputs):
        # 1. 解包输入：
        #   inputs[0]: 来自f_s_attention的碎片-口袋整合特征 [B, 256]
        #   inputs[1]: 药物-蛋白质特征dp [B, 256]
        fs = inputs[0]  # [B, 256]
        dp_embed = inputs[1]  # [B, 256]
        # print(f"fs shape: {fs.shape}")  # 添加打印
        # print(f"dp_embed shape: {dp_embed.shape}")  # 添加打印

        # 2. 直接拼接特征
        combined = torch.cat([fs, dp_embed], dim=1)  # [B, 512]
        # print(f"combined shape: {combined.shape}")  # 添加打印
        # 3. 通过解码器得到最终输出
